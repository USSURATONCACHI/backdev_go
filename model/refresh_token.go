...
func (model *Model) RefreshToken(tokenString string, refreshTokenBase64 string, userIp string) (*JwtAndRefreshTokens, error, error) {
	...
	if claims.UserIp != userIp {
		go model.refresh_ProcessIpMismatch(claims.UserIp, userIp, claims.UserName, claims.UserEmail)
	}
...
func (model *Model) refresh_ProcessIpMismatch(oldIp string, newIp string, userName string, email string) {
